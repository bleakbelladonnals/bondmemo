...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        RateLimiter::for('relationship-agent', function ($request) {
            return Limit::perMinute(max(1, config('relationship_agent.requests_per_minute')))
                ->by($request->user()->id);
        });
    ...
    {
        $this->app->bind(
            \App\Interfaces\RelationshipAgentClient::class,
            \App\Services\RelationshipAgent\OpenAICompatibleRelationshipAgentClient::class
        );
