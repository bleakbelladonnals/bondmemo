...
echo -e "\n\033[4;32mInstalling PHP 8.2\033[0;40m"
apt-get install -y curl gnupg2 apt-transport-https apt-transport-https lsb-release ca-certificates >/dev/null
add-apt-repository -y ppa:ondrej/php >/dev/null
apt-get update >/dev/null
apt-get install -y php8.2 >/dev/null

echo -e "\n\033[4;32mInstalling git\033[0;40m"
apt-get install -y git >/dev/null

echo -e "\n\033[4;32mInstalling composer\033[0;40m"
apt-get install -y curl php8.2-cli >/dev/null
curl -sS https://getcomposer.org/installer | php -- --install-dir=/usr/local/bin --filename=composer >/dev/null

echo -e "\n\033[4;32mInstalling packages for Monica\033[0;40m"
apt-get install -y php8.2-bcmath php8.2-curl php8.2-common php8.2-fpm \
    php8.2-gd php8.2-gmp php8.2-intl php8.2-mbstring php8.2-mysql \
    php8.2-opcache php8.2-redis php8.2-xml php8.2-zip >/dev/null
